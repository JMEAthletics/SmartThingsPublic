...
@Field static Map PRESET_PROGRAMS = [fireplace:6, storm:7, rainbow:8, Aurora:9, police:10]
@Field static List DIMMER_NAMES = ["white", "color", "red", "green", "blue"]


metadata {
	definition (name: "Fibaro RGBW Dimmer", namespace: "krlaframboise", author: "Kevin LaFramboise", ocfDeviceType: "oic.d.light", vid: "generic-rgbw-color-bulb") {
		capability "Actuator"
		capability "Sensor"
		...
		command "startAuroraProgram"
		command "startPoliceProgram"
		command "startProgram", ["NUMBER"]
		command "stopProgram"

		fingerprint mfr:"010F", prod:"0900", model:"3000", deviceJoinName:"Fibaro RGBW Dimmer"
	}
	...
	tiles(scale:2) {
		...
		standardTile("Aurora", "device.activeProgram", width: 2, height: 2) {
			state "default", label:'Aurora', action: "startAuroraProgram", backgroundColor:"#ffffff"
			state "9", label:'Aurora', action: "stopProgram", backgroundColor:"#00a0dc"
		}
		...
		main(["switch"])
		details(["switch", "whiteLabel", "whiteSlider", "whiteSwitch", "redLabel", "redSlider", "redSwitch", "greenLabel", "greenSlider", "greenSwitch", "blueLabel", "blueSlider", "blueSwitch", "colorLabel", "colorSlider", "colorSwitch", "power", "syncStatus", "firmwareVersion", "refresh", "configure", "stop", "fireplace", "storm", "rainbow", "Aurora", "police", "rgbSelector"])
	}
	...
	preferences {		
		...
		getBoolInput("createWhiteDimmer", "Create Child Dimmer for White?", true)
		getBoolInput("createColorDimmer", "Create Child Dimmer for Color?\n(Allows you to control Red, Green, and Blue together.)", false)
		getBoolInput("createRedDimmer", "Create Child Dimmer for Red?", true)
		getBoolInput("createGreenDimmer", "Create Child Dimmer for Green?", true)
		getBoolInput("createBlueDimmer", "Create Child Dimmer for Blue?", true)
		...
		getBoolInput("createFireplaceSwitch", "Create Child Switch for Fireplace Program?", true)
		getBoolInput("createStormSwitch", "Create Child Switch for Storm Program?", true)
		getBoolInput("createRainbowSwitch", "Create Child Switch for Rainbow Program?", true)
		getBoolInput("createAuroraSwitch", "Create Child Switch for Aurora Program?", true)
		getBoolInput("createPoliceSwitch", "Create Child Switch for Police Program?", true)
...
def startAuroraProgram() { return startProgram(9) }
